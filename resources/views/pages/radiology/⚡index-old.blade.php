<?php

use App\Models\User;
use App\Models\Modalidad;
use Livewire\Component;
use Livewire\WithPagination;
use Livewire\Attributes\On;
use Livewire\Attributes\Title;
use Livewire\Attributes\Computed;


new #[Title('Modalities')] class extends Component
{
    use WithPagination;

    public array $options = [

        'search' => '',

        'searchFields' => [
            'aet' => [
                'label' => 'AE Title',
                'enabled' => true,
            ],
            'ip' => [
                'label' => 'Dirección IP',
                'enabled' => true,
            ],
            'location' => [
                'label' => 'Ubicación',
                'enabled' => true,
            ],
            'department' => [
                'label' => 'Servicio',
                'enabled' => true,
            ],
            'model' => [
                'label' => 'Modelo',
                'enabled' => true,
            ],
            'station' => [
                'label' => 'Estacion',
                'enabled' => true,
            ],
        ],

        'filters' => [
            'department' => [],
            'centre'     => [],
            'modalidad'  => [],
            'syngo'      => [],
        ],

        'sort' => [
            'by' => 'aet',
            'direction' => 'asc',
        ],
    ];


    public function mount()
    {
        $this->authorize('viewAny', Modalidad::class);
    }

    public function sort($column) {
        if ($this->sortBy === $column) {
            $this->sortDirection = $this->sortDirection === 'asc' ? 'desc' : 'asc';
        } else {
            $this->sortBy = $column;
            $this->sortDirection = 'asc';
        }
        $this->resetPage();
    }

    public function updatedSearch()
    {
        $this->resetPage();
    }

    public function updatedFilters()
    {
        $this->resetPage();
    }

    public function resetFilters()
    {
        $this->filters = [
            'department' => null,
            'modalidad'  => null,
            'syngo'      => null,
        ];

        $this->resetPage();
    }

    #[On('modality-created')]
    #[On('modality-updated')]
    #[On('modality-deleted')]
    #[On('modalities-imported')]
    public function refresh()
    {
        $this->resetPage();
    }

    #[Computed]
    public function modalidades()
    {
        $query = Modalidad::query();

        $search = trim($this->options['search']);

        if ($search !== '') {

            $search = "%{$search}%";

            $query->where(function ($q) use ($search) {

                foreach ($this->options['searchFields'] as $field => $enabled) {
                    if ($enabled) {
                        $q->orWhere($field, 'like', $search);
                    }
                }

            });
        }

        $departments = collect($this->options['filters']['department'] ?? [])
            ->filter()
            ->keys()
            ->values()
            ->toArray();

        if (!empty($departments)) {
            $query->whereIn('department', $departments);
        }

        $modalidades = collect($this->options['filters']['modalidad'] ?? [])
            ->filter()
            ->keys()
            ->values()
            ->toArray();

        if (!empty($modalidades)) {
            $query->whereIn('modalidad', $modalidades);
        }

        $query->orderBy(
            $this->options['sort']['by'],
            $this->options['sort']['direction']
        );

        return $query->paginate(10);
    }


};
?>

<section class="w-full">
    <x-pages::radiology.layout :heading="__('rx.inventory_heading')" :subheading="__('rx.inventory_subheading')">
        <div class="border-1 p-4 rounded-md border-zinc-200 dark:border-zinc-600 bg-zinc-50 dark:bg-zinc-700">
            <div class="flex mb-4 space-x-2">
                <flux:input.group class="flex-2">
                    <flux:input icon="magnifying-glass" 
                        wire:model.live.debounce.300ms="search"
                        placeholder="{{ __('rx.search_modality') }}..."
                        clearable
                    />

<flux:dropdown>
    <flux:button icon:trailing="chevron-down">Campo</flux:button>

    <flux:menu keep-open>
        @foreach($searchFields as $field => $config)
        <flux:menu.checkbox wire:model="searchFields.{{ $field }}.enabled">{{ $config['label'] }}</flux:menu.checkbox>
        @endforeach
    </flux:menu>
</flux:dropdown>

                </flux:input.group>

<flux:dropdown>
    <flux:button icon:trailing="chevron-down">Opciones</flux:button>
    <flux:menu>
        <flux:menu.submenu heading="Ordenar por">
            <flux:menu.radio>aet</flux:menu.radio>
            <flux:menu.radio>service</flux:menu.radio>
            <flux:menu.radio>centre</flux:menu.radio>
        </flux:menu.submenu>
        <flux:menu.submenu heading="Filtros">
            <flux:menu.submenu heading="Department">
                <flux:menu.checkbox>1</flux:menu.checkbox>
                <flux:menu.checkbox>2</flux:menu.checkbox>
                <flux:menu.checkbox>3</flux:menu.checkbox>
            </flux:menu.submenu>
            <flux:menu.submenu heading="Modalidad">
                <flux:menu.checkbox>1</flux:menu.checkbox>
                <flux:menu.checkbox>2</flux:menu.checkbox>
                <flux:menu.checkbox>3</flux:menu.checkbox>
            </flux:menu.submenu>            
        </flux:menu.submenu>
    </flux:menu>
</flux:dropdown>


<flux:dropdown>

    <flux:button icon:trailing="chevron-down" variant="ghost">
        Opciones
    </flux:button>

    <flux:menu keep-open class="w-64">

        {{-- ORDENAR --}}
        <flux:menu.submenu keep-open heading="Ordenar por">

            <flux:menu.radio
                wire:model="sortBy"
                value="aet"
            >
                AET
            </flux:menu.radio>

            <flux:menu.radio
                wire:model="sortBy"
                value="department"
            >
                Departamento
            </flux:menu.radio>

            <flux:menu.radio
                wire:model="sortBy"
                value="centre"
            >
                Centro
            </flux:menu.radio>

            <flux:menu.separator />

            {{-- Dirección --}}
            <flux:menu.radio
                wire:model="sortDirection"
                value="asc"
            >
                Ascendente
            </flux:menu.radio>

            <flux:menu.radio
                wire:model="sortDirection"
                value="desc"
            >
                Descendente
            </flux:menu.radio>

        </flux:menu.submenu>

        {{-- FILTROS --}}
        <flux:menu.submenu keep-open heading="Filtros">

            {{-- Department --}}
            <flux:menu.submenu keep-open wire:key="department-filter-wrapper" heading="Departamento">

                @foreach(
                    \App\Models\Modalidad::select('department')
                        ->distinct()
                        ->pluck('department')
                        ->filter()
                        ->sort()
                        ->values()
                    as $dep
                )
                    <flux:menu.checkbox
                        wire:key="filter-department-{{ md5($dep) }}"
                        wire:model="filters.department.{{ $dep }}"
                        value="{{ $dep }}"
                    >
                        {{ $dep }}
                    </flux:menu.checkbox>
                @endforeach

            </flux:menu.submenu>

            {{-- Modalidad --}}
            <flux:menu.submenu keep-open heading="Modalidad">

                @foreach(
                    \App\Models\Modalidad::select('modalidad')
                        ->distinct()
                        ->pluck('modalidad')
                        ->filter()
                        ->sort()
                    as $mod
                )
                    <flux:menu.checkbox
                        wire:model="filters.modalidad"
                        value="{{ trim($mod) }}"
                    >
                        {{ $mod }}
                    </flux:menu.checkbox>
                @endforeach

            </flux:menu.submenu>

        </flux:menu.submenu>

    </flux:menu>

</flux:dropdown>



                <flux:spacer />

                <livewire:radiology.import />

                <livewire:radiology.export 
                    :search="$search"
                    :sortBy="$sortBy"
                    :sortDirection="$sortDirection"
                />

                @can('create', App\Models\Modalidad::class)
                    <flux:button variant="primary"
                        wire:click="$dispatch('create-modality')"
                    >
                        {{ __('rx.add_modality') }}
                    </flux:button>
                @endcan
            </div>   

            <div class="flex flex-row">
                <flux:select wire:model="filters.department" placeholder="Departamento">
                    <flux:select.option value="">Todos</flux:select.option>
                    @foreach(\App\Models\Modalidad::select('department')->distinct()->pluck('department') as $dep)
                        @if($dep)
                            <flux:select.option value="{{ $dep }}">{{ $dep }}</flux:select.option>
                        @endif
                    @endforeach
                </flux:select>

                <flux:select wire:model="filters.modalidad" placeholder="Modalidad">
                    <flux:select.option value="">Todas</flux:select.option>
                    @foreach(\App\Models\Modalidad::select('modalidad')->distinct()->pluck('modalidad') as $mod)
                        @if($mod)
                            <flux:select.option value="{{ $mod }}">{{ $mod }}</flux:select.option>
                        @endif
                    @endforeach
                </flux:select>

                <flux:select wire:model="filters.syngo" placeholder="Syngo">
                    <flux:select.option value="">Todos</flux:select.option>
                    <flux:select.option value="1">Conectado</flux:select.option>
                    <flux:select.option value="0">No conectado</flux:select.option>
                </flux:select>

                <flux:button variant="ghost" wire:click="resetFilters">
                    Limpiar filtros
                </flux:button>
            </div>    

            <div class="flex-col space-y-4 mt-6">
                @forelse($this->modalidades as $modalidad)
                    @include('partials.radiology.modalidad')
                @empty
                    <div class="col-span-full text-center text-gray-500 py-10">
                        No se encontraron modalidades
                    </div>
                @endforelse
            </div>

            {{-- Paginación --}}
            <div class="mt-6">
                {{ $this->modalidades->links() }}
            </div>
        </div>

        <livewire:radiology.create />
        <livewire:radiology.edit />
    </x-pages::radiology.layout>
</section>